...
def adjust_state_dict(state_dict, regtoken_path="longregtokens"):
    new_state_dict = {}
...
def build_model(state_dict: dict, regtoken_path="longregtokens"):
    ...
    print(f"Checking Text Encoder positional_embedding before model init: {context_length}")
    ...
    new_text_embed = state_dict["positional_embedding"].shape[0]    
    ...
    print(f"Checking Text Encoder positional_embedding after model init: {new_text_embed}")
    ...
    if new_text_embed != 248:
        state_dict["positional_embedding"] = nn.Parameter(torch.empty(248, transformer_width))
        state_dict["positional_embedding_res"] = nn.Parameter(torch.empty(248, transformer_width))        
