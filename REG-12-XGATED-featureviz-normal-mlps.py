...
def parse_arguments():
    ...
    parser.add_argument('--layer_range', default="14-23", type=str, help="Which layers to visualize; continuous range ('5-10') or discrete values ('5,6,8'). Default: 14-23")
    ...
    parser.add_argument("--output_folder", default='FeatureViz/NORMAL-MLP', help="Folder to save output image; default: FeatureViz/NORMAL-MLP")
...
def generate_visualizations(model, clipname, layer_range_str, feature_range_str, image_size, tv, lr, steps, print_every, save_every, saver, coefficient):
    ...
    for layer in layer_range:
        for feature in feature_range:
            ...
            loss = LossArray()
            loss += ViTREGEnsFeatHook(REGClipGeLUHook(model, sl=slice(layer, layer + 1)), key='high', feat=feature, coefficient=1)
