...
import argparse
import torch
from regtransformers import CLIPConfig, CLIPModel  # transformers v4.52.3 from 22/May/2025
# Changes made to:
# regtransformers/models/clip/configuration_clip.py
# regtransformers/models/clip/modeling_clip.py

# Adapted for CLIP-fine-tune-registers-gated
# https://github.com/zer0int/CLIP-fine-tune-registers-gated
from longINFERclipregXGATED import load  # Modified OpenAI-CLIP ViT with register tokens + fusion MLP Gates
...
def convert_clipreg_checkpoint(checkpoint_path, pytorch_dump_folder_path, config_path=None):
    ...
    hf_model = CLIPModel(config).eval().float()
    ...
    # After copying all weights but before assertion
    pt_image_embeds, pt_text_embeds = pt_model.encode_image(pixel_values), pt_model.encode_text(input_ids)
    hf_image_embeds, hf_text_embeds = hf_model.get_image_features(pixel_values), hf_model.get_text_features(input_ids)

    print("Max diff image_embeds", (pt_image_embeds - hf_image_embeds).abs().max())
    print("Max diff text_embeds", (pt_text_embeds - hf_text_embeds).abs().max())

    # Check after normalization
    print("Norms (PT, HF):", pt_image_embeds.norm(dim=-1), hf_image_embeds.norm(dim=-1))
    
    # Skip due to interpolation of embeds leading to Max diff text_embeds tensor(0.0097) (vs. image_embeds tensor(1.9073e-06) <- stochastic noise)
